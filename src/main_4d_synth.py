...
def run_synth_4d(num_samples):
    ...
    OPTIMIZER = "ADAM"
    learning_rate_classifier = 1#.1 #1#.05
    learning_rate_gates = .05#.05 #.05#.5
    
    batch_size = len(normalized_samples)#100
    ...
    for epoch in range(n_epoch):
        ...
        for i in range(n_mini_batch):
            ...
            if n_mini_batch_update_gates == 0:
                optimizer_gates.step()
                optimizer_classifier.step()
            elif (n_mini_batch * epoch + i) % n_mini_batch_update_gates == 0:
                optimizer_gates.step()
            else:
                optimizer_classifier.step()
